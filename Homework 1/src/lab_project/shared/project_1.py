...
class Project_1( object ):
    def __init__( self, burst, capacity, collectors, cutoff, hard, inactivity, k, period, version ):
        ...
        # Defining the internal topology mapping logic ( 1 -> custom dictionary, 2 -> "networkx" graph )
        self.version = int( version )
        log.info( "[SYSTEM] Starting Project_1 using mode: %s", self.version )
        
        # Initializing the global topology based on the selected version
        if self.version == 1:
            self.topology = {} 
        else:
        ...
        if path not in self.path_cache:
            if self.version == 1:
                ...
                try:
                    ...
                    self.path_cache[ path ] = paths[ :self.k ] 
